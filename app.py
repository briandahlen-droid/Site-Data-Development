...
if missing_files:
    ...
    The following files are missing from the application directory:
    {', '.join(missing_files)}
    
    Please ensure all files from the deployment package are uploaded:
    - app.py
    - county_adapters.py
    - excel_generator.py
    - municode_parser.py
...
try:
    ...
    Please ensure all Python files are in the same directory as app.py
    """)
    st.stop()
